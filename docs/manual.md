...
[← 返回首页](../README.md) · [下载最新版](https://github.com/zeronchen/md-viewer/releases/latest)
...
| **隐藏目录** | `.claude`、`.github`、`.trellis` 这类点号目录，浅层探到 Markdown 就显示在文件树与搜索中（`.git`、`node_modules` 等永不显示） |
...
2. 下载 `MD-Viewer-Setup-3.1.5.exe`（安装版）或 `MD-Viewer-Folder-3.1.5.zip`（免安装版，解压即用）
